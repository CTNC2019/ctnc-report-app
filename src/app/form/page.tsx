...
const inputCls = "w-full px-4 py-3 bg-canvas border border-border-subtle rounded-xl text-ink focus:ring-2 focus:ring-primary-500 focus:outline-none";
const labelCls = "block text-sm font-medium text-ink mb-2";
const cardCls = "p-6 bg-canvas border border-border-subtle rounded-2xl relative";
...
function FormInner() {
  ...
  if (!loaded || !masterData) {
    return <div className="min-h-screen bg-canvas flex items-center justify-center text-ink-secondary">{t("common.loading")}</div>;
  }
    ...
    <div className="min-h-screen bg-canvas p-4 sm:p-8">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center gap-4 mb-6">
          <Link href="/reports" className="p-2 rounded-full hover:bg-canvas transition-colors">
            <ArrowLeft className="w-6 h-6 text-ink-secondary" />
          </Link>
          <div>
            <h1 className="text-3xl font-bold text-ink tracking-tight">{t("form.title")}</h1>
            <p className="text-primary-700 text-sm mt-1">{stepTitles[currentStep - 1]} · {currentStep}/{totalSteps}</p>
          </div>
        </div>

        <div className="w-full h-1.5 bg-slate-200 rounded-full mb-8 overflow-hidden">
          <motion.div className="h-full bg-primary-600" initial={{ width: "0%" }} animate={{ width: `${(currentStep / totalSteps) * 100}%` }} transition={{ duration: 0.3 }} />
        </div>

        <div className="bg-surface border border-border-subtle rounded-3xl p-6 sm:p-8 shadow-lg">
          <AnimatePresence mode="wait">
            {/* Step 1: General */}
            {currentStep === 1 && (
              <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                <h2 className="text-2xl font-bold text-ink mb-2">{t("form.step.general")}</h2>
            ...
            {currentStep === 2 && site && (
              <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                <h2 className="text-2xl font-bold text-ink mb-2">{t("form.step.sites")}</h2>
                ...
                <div className={cardCls}>
                  ...
                  <div className="space-y-3 mb-4">
                    <div className="flex items-center justify-between">
                      ...
                      <button onClick={() => addActivity(activeSite)} className="flex items-center gap-1.5 bg-primary-600 hover:bg-primary-700 px-3 py-1.5 rounded-lg text-xs text-white">
                        <Plus className="w-3.5 h-3.5" /> {t("form.addActivity")}
                      </button>
                    </div>
                    {site.activities.length === 0 && <p className="text-sm text-ink-muted">{t("form.noItems")}</p>}
                    {site.activities.map((a) => (
                      <div key={a.key} className="flex flex-col sm:flex-row gap-2 items-start sm:items-center bg-white border border-border-subtle rounded-xl p-3">
                        <select value={a.activityType} onChange={(e) => updateActivity(activeSite, a.key, { activityType: e.target.value })} className="px-3 py-2 bg-canvas border border-border-subtle rounded-lg text-ink text-sm sm:w-56">
                          {masterData.activityTypes.map((x) => <option key={x.code} value={x.code}>{L(x)}</option>)}
                        </select>
                        <input type="text" placeholder={t("form.activityDesc")} value={a.desc} onChange={(e) => updateActivity(activeSite, a.key, { desc: e.target.value })} className="flex-1 px-3 py-2 bg-canvas border border-border-subtle rounded-lg text-ink text-sm w-full" />
                        <button onClick={() => removeActivity(activeSite, a.key)} className="p-2 text-danger hover:bg-danger-soft rounded-lg shrink-0">
                  ...
                  <div className="mb-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 bg-primary-600 hover:bg-primary-700 px-3 py-1.5 rounded-lg text-xs text-white disabled:opacity-60"
                      >
                    ...
                    {site.photos.length === 0 && <p className="text-sm text-ink-muted">{t("form.noItems")}</p>}
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      {site.photos.map((p) => (
                        <div key={p.url} className="relative bg-white rounded-xl overflow-hidden border border-border-subtle">
                          <button onClick={() => removePhoto(activeSite, p.url)} className="absolute top-1.5 right-1.5 z-10 p-1 bg-black/60 text-white rounded-full hover:bg-black/80">
                          ...
                          <input
                            ...
                            className="w-full px-2 py-1.5 bg-canvas text-xs text-ink border-t border-border-subtle focus:outline-none"
                          />
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="mb-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button onClick={() => addDoc(activeSite)} className="flex items-center gap-1.5 bg-primary-600 hover:bg-primary-700 px-3 py-1.5 rounded-lg text-xs text-white">
                        <Plus className="w-3.5 h-3.5" /> {t("form.addDoc")}
                      </button>
                    </div>
                    {site.relatedDocs.length === 0 && <p className="text-sm text-ink-muted">{t("form.noItems")}</p>}
                    <div className="space-y-2">
                      {site.relatedDocs.map((d, i) => (
                        <div key={i} className="flex flex-col sm:flex-row gap-2 items-start sm:items-center bg-white border border-border-subtle rounded-xl p-3">
                          <input
                            type="text"
                            placeholder="https://..."
                            value={d.url}
                            onChange={(e) => updateDoc(activeSite, i, { url: e.target.value })}
                            className="flex-1 px-3 py-2 bg-canvas border border-border-subtle rounded-lg text-ink text-sm w-full"
                          />
                          <input
                            type="text"
                            ...
                            className="px-3 py-2 bg-canvas border border-border-subtle rounded-lg text-ink text-sm sm:w-56"
                          />
                          <button onClick={() => removeDoc(activeSite, i)} className="p-2 text-danger hover:bg-danger-soft rounded-lg shrink-0">
                            <Trash2 className="w-4 h-4" />
                          </button>
                        </div>
                      ))}
            ...
            {currentStep === 3 && (
              <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                <div className="flex justify-between items-center mb-2">
                  <h2 className="text-2xl font-bold text-ink">{t("form.step.proposals")}</h2>
                  <button onClick={addProposal} className="flex items-center gap-2 bg-canvas hover:bg-slate-200 px-4 py-2 rounded-lg text-sm text-ink border border-border-subtle">
            ...
            {currentStep === 4 && (
              <motion.div key="s4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                <div className="flex justify-between items-center mb-2">
                  <h2 className="text-2xl font-bold text-ink">{t("form.step.reportsdata")}</h2>
                  <button onClick={addReportItem} className="flex items-center gap-2 bg-canvas hover:bg-slate-200 px-4 py-2 rounded-lg text-sm text-ink border border-border-subtle">
            ...
            {currentStep === 5 && (
              <motion.div key="s5" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                <h2 className="text-2xl font-bold text-ink mb-2">{t("form.step.comms")}</h2>
                {comms.map((c, i) => {
                  const meta = masterData.commChannels[i];
                  return (
                    <div key={c.channelCode} className={cardCls}>
                      <h3 className="text-base font-bold text-ink mb-4">{L(meta)}</h3>
            ...
            {currentStep === 6 && (
              <motion.div key="s6" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                <div className="flex justify-between items-center mb-2">
                  <h2 className="text-2xl font-bold text-ink">{t("form.step.issues")}</h2>
                  <button onClick={addIssue} className="flex items-center gap-2 bg-canvas hover:bg-slate-200 px-4 py-2 rounded-lg text-sm text-ink border border-border-subtle">
            ...
            {currentStep === 7 && (
              <motion.div key="s7" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                <div className="flex justify-between items-center mb-2">
                  <h2 className="text-2xl font-bold text-ink">{t("form.step.priorities")}</h2>
                  <button onClick={addPriority} disabled={priorities.length >= 6} className="flex items-center gap-2 bg-canvas hover:bg-slate-200 disabled:opacity-40 px-4 py-2 rounded-lg text-sm text-ink border border-border-subtle">
                ...
                {priorities.map((x, i) => (
                  <div key={x.key} className={cardCls + " group"}>
                    ...
                    <div className="flex items-center gap-2 mb-4">
                      <span className="w-8 h-8 rounded-full bg-primary-600 text-white font-bold flex items-center justify-center text-sm">{i + 1}</span>
                      <span className="text-sm text-ink-secondary">{t("form.priorityNo")}</span>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <Field label={t("form.site")}>
            ...
            {currentStep === 8 && (
              <motion.div key="s8" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                <div className="flex justify-between items-center mb-2">
                  <h2 className="text-2xl font-bold text-ink">{t("form.step.deadlines")}</h2>
                  <button onClick={addDeadline} className="flex items-center gap-2 bg-canvas hover:bg-slate-200 px-4 py-2 rounded-lg text-sm text-ink border border-border-subtle">
            ...
            {currentStep === 9 && !done && (
              <motion.div key="s9-confirm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                ...
                <div className="p-5 bg-canvas border border-border-subtle rounded-2xl text-sm text-ink-secondary space-y-1">
                  ...
                  <p className="text-ink-secondary">{t("form.step.reportsdata")}: {reportItems.length}</p>
                  <p className="text-ink-secondary">{t("form.step.issues")}: {issues.length}</p>
                  <p className="text-ink-secondary">{t("form.step.priorities")}: {priorities.length}</p>
                  <p className="text-ink-secondary">{t("form.step.deadlines")}: {deadlines.length}</p>
                </div>
              </motion.div>
            )}
            ...
            {currentStep === 9 && done && (
              <motion.div key="s9-done" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center justify-center py-12 text-center">
                <div className="w-20 h-20 bg-primary-600 rounded-full flex items-center justify-center mb-6 shadow-[0_0_30px_rgba(27,94,32,0.25)]">
                  <CheckCircle className="w-10 h-10 text-white" />
                </div>
                <h2 className="text-3xl font-bold text-ink mb-2">{t("form.success.title")}</h2>
                <p className="text-ink-secondary max-w-md mb-8">{t("form.success.desc")}</p>
                <Link href="/dashboard">
                  <button className="bg-canvas hover:bg-slate-200 text-ink px-6 py-3 rounded-xl transition-colors font-medium border border-border-subtle">{t("form.backDash")}</button>
                </Link>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          {!(currentStep === 9 && done) && (
            <div className="flex justify-between mt-10 pt-6 border-t border-border-subtle">
              <button onClick={prevStep} className="px-6 py-2.5 rounded-xl font-medium text-ink-secondary hover:bg-canvas transition-colors">
                {t("form.back")}
              </button>
              {currentStep < 9 ? (
                <button onClick={nextStep} className="flex items-center gap-2 bg-primary-600 hover:bg-primary-700 text-white px-6 py-2.5 rounded-xl font-bold shadow-lg transition-transform hover:scale-105 active:scale-95">
                  {t("form.next")} <ArrowRight className="w-4 h-4" />
                </button>
              ) : (
                <div className="flex gap-3">
                  <button onClick={() => persist("Draft")} disabled={isSubmitting} className="flex items-center gap-2 bg-canvas hover:bg-slate-200 text-ink px-5 py-2.5 rounded-xl font-medium disabled:opacity-70 border border-border-subtle">
                    <Save className="w-4 h-4" /> {t("form.saveDraft")}
                  </button>
                  <button onClick={() => persist("Submitted")} disabled={isSubmitting} className="flex items-center gap-2 bg-accent-blue hover:bg-blue-800 text-white px-8 py-2.5 rounded-xl font-bold shadow-lg transition-transform hover:scale-105 active:scale-95 disabled:opacity-70">
...
function RemoveBtn({ onClick }: { onClick: () => void }) {
  return (
    <button onClick={onClick} className="absolute top-4 right-4 p-2 text-danger hover:bg-danger-soft rounded-lg opacity-60 hover:opacity-100 transition-opacity">
...
function EmptyHint({ text }: { text: string }) {
  return (
    <div className="text-center p-8 bg-canvas border border-border-subtle rounded-xl border-dashed">
